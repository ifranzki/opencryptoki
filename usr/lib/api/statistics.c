                                  ...
                                  CK_ULONG strength_idx)
{
    ...
    case CKM_IBM_ML_KEM_WITH_ECDH:
        ...
        if (mech->pParameter == NULL)
            return CKR_MECHANISM_PARAM_INVALID;
        if (((CK_IBM_ML_KEM_WITH_ECDH_PARAMS *)mech->pParameter)->kdf == CKD_NULL ||
            ((CK_IBM_ML_KEM_WITH_ECDH_PARAMS *)mech->pParameter)->kdf == CKD_IBM_HYBRID_NULL)
            break;
        rc = digest_from_kdf(((CK_IBM_ML_KEM_WITH_ECDH_PARAMS *)mech->pParameter)->kdf,
                             &implicit_mech.mechanism);
        if (rc != CKR_OK)
            return rc;
        rc = statistics_increment(statistics, slot, &implicit_mech,
