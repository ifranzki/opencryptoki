 ...
 * CCA only supports to derive AES and DES keys.
 */
                                                                 ...
                                                                 *derive_info)
{
    ...
    case CKK_AES:
       switch (value_len) {
        ...
        break;
    default:
        TRACE_ERROR("CCA does not support to derive keys of type 0x%lx as DATA keys\n",
                    key_type);
        return CKR_ATTRIBUTE_VALUE_INVALID;
    }
...
static CK_RV import_ec_privkey(STDLL_TokData_t *tokdata, TEMPLATE *priv_templ)
{
    ...
    if (rc == TRUE) {
        ...
        /*
         * The CCA token only supports to derive DATA keys. So if the ECC
         * private key token has an ECC key derivation info section, the
         * to-be-derived key type must be DATA.
        ...
        if (ecc_info != NULL &&
            ...
            TRACE_ERROR("ECC private key has a ECC key derivation info section "
                        "(X'23'), but the key type is not DATA\n");
            return CKR_ATTRIBUTE_VALUE_INVALID;
        }
 ...
 * of key that can be derived with that key.
 * The CCA token generates or imports ECC keys with a ECC key derivation
 ...
 * using the CKA_DERIVE_TEMPLATE containing CKA_KEY_TYPE and CKA_VALUE_LEN.
                                          ...
                                          CK_ULONG derived_key_type)
{
    ...
    unsigned char key_type[CCA_KEYWORD_SIZE];
    unsigned char symkey_token[CCA_KEY_ID_SIZE] = { 0, };
    long symkey_token_length = sizeof(symkey_token);
    long shared_data_length, privkey_token_length;
    unsigned char param3[4] = { 0, };
    unsigned char mkvp2[16] = { 0, };
    CK_ATTRIBUTE *opaque_attr, *attr;
    ...
    switch (params->kdf) {
        ...
        case CKD_SHA512_KDF:
            break;
        default:
            TRACE_ERROR("CCA does not support KDFs other than SHA224/256/384/521\n");
            return CKR_MECHANISM_PARAM_INVALID;
    }

    if (params->ulSharedDataLen > 256) {
        TRACE_ERROR("CCA does not shared data > 256 bytes\n");
        return CKR_MECHANISM_PARAM_INVALID;
    }
    ...
    ecc_info = cca_ec_ecc_key_derivation_info(opaque_attr->pValue);
    if (ecc_info == NULL) {
        TRACE_ERROR("ECC private key does not have an ECC key derivation info section X'23'\n");
        return CKR_KEY_FUNCTION_NOT_PERMITTED;
    }

    /* Check the derived key: Must match the ECC key derivation info */
    if (ecc_info->key_type != CCA_KEY_DERIVE_TYPE_DATA) {
        TRACE_ERROR("CCA can not derive keys other than DATA key tokens\n");
        return CKR_KEY_TYPE_INCONSISTENT;
    }

    switch (ecc_info->key_algorithm) {
    case CCA_AES_KEY:
        ...
        if (key_len != ecc_info->key_size / 8) {
            TRACE_ERROR("CCA can not derive keys of type other than AES-%u\n",
                        ecc_info->key_size);
            return CKR_ATTRIBUTE_VALUE_INVALID;
        }
        break;
    default:
        TRACE_ERROR("Unsupported ECC key derivation key algorithm\n");
        return CKR_KEY_TYPE_INCONSISTENT;
    }
    ...
    rule_array_count = 4;
    memcpy(rule_array, "INTERNALNO-KEY  ", CCA_KEYWORD_SIZE * 2);

    switch (key_len) {
    case 16:
        memcpy(rule_array + 2 * CCA_KEYWORD_SIZE, "AES     KEYLN16 ",
               CCA_KEYWORD_SIZE * 2);
        break;
    case 24:
        memcpy(rule_array + 2 * CCA_KEYWORD_SIZE, "AES     KEYLN24 ",
               CCA_KEYWORD_SIZE * 2);
        break;
    case 32:
        memcpy(rule_array + 2 * CCA_KEYWORD_SIZE, "AES     KEYLN32 ",
               CCA_KEYWORD_SIZE * 2);
        break;
    default:
        TRACE_ERROR("Unsupported key size\n");
    ...
    /* Add CKA_IBM_OPAQUE */
    rc = build_attribute(CKA_IBM_OPAQUE, symkey_token, symkey_token_length,
                         &attr);
    if (rc != CKR_OK) {
    ...
    rc = build_attribute(CKA_VALUE, dummy, key_len, &attr);
    if (rc != CKR_OK) {
        TRACE_ERROR("%s build_attribute failed with rc=0x%lx\n", __func__, rc);
        goto done;
    }
                                          ...
                                          TEMPLATE *new_tmpl)
{
    ...
    CK_OBJECT_CLASS class;
    CK_KEY_TYPE ktype, derive_key_type;
    ...
    if (rc == CKR_OK && derive == TRUE) {
         ...
         * Check if the the key has an ECC key derivation section (X'23').
    ...
    if (template_attribute_find(new_tmpl, CKA_DERIVE_TEMPLATE,
         ...
         * Check if the the key has an ECC key derivation section (X'23'),
         * and if it matches the CKA_KEY_TYPE and CKA_VALUE_LEN attributes
         * in the derive template.
         */
        rc = ccatok_get_key_info_from_derive_template(new_tmpl,
                                                      &derive_key_type,
                                                      &derive_value_len);
        if (rc == CKR_OK && ecc_info != NULL &&
            derive_key_type != (CK_ULONG)-1) {
            switch (derive_key_type) {
            case CKK_AES:
                if (ecc_info->key_algorithm != CCA_AES_KEY) {
                    TRACE_ERROR("The EC private key can not derive keys other "
                                "than AES keys\n");
                    return CKR_ATTRIBUTE_VALUE_INVALID;
                }
