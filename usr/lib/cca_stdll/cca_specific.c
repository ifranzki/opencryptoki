...
static const MECH_LIST_ELEMENT cca_mech_list[] = {
    ...
    {CKM_RSA_PKCS, {512, 4096, CKF_HW | CKF_ENCRYPT | CKF_DECRYPT | CKF_SIGN |
                    CKF_VERIFY | CKF_WRAP | CKF_UNWRAP | CKF_ENCAPSULATE |
                    CKF_DECAPSULATE}},
    ...
    {CKM_RSA_PKCS_OAEP, {512, 4096, CKF_HW | CKF_ENCRYPT | CKF_DECRYPT |
                         CKF_WRAP | CKF_UNWRAP | CKF_ENCAPSULATE |
                         CKF_DECAPSULATE}},
    {CKM_DES_CBC,
    ...
    {CKM_ML_KEM, {800, 1568, CKF_HW | CKF_ENCAPSULATE | CKF_DECAPSULATE}},
                                           ...
                                           CK_ULONG keyform)
{
    ...
    case CKM_ML_KEM_KEY_PAIR_GEN:
        ...
        break;
    case CKM_ML_KEM:
        required = &cca_v8_4; // TODO: Will probably need 8.5 ?
        break;
    default:
        return FALSE;
    }
                                                ...
                                                CK_MECHANISM_INFO *info)
{
    ...
    case CKM_ML_KEM_KEY_PAIR_GEN:
    case CKM_ML_KEM:
        rc = cca_pqc_strength_supported(tokdata, mechanism, CKP_ML_KEM_768);
        break;
                             ...
                             uint16_t *curve_bitlen, int *curve_nid)
{
    ...
    rc = template_attribute_get_ulong(templ, CKA_KEY_TYPE, &key_type);
    if (rc != CKR_OK) {
    ...
    for (i = 0; i < NUMEC; i++) {
        if ((attr->ulValueLen == der_ec_supported[i].data_size) &&
            ...
            /* Check if the curves fits to the key type */
            switch (key_type) {
            case CKK_EC:
                if (der_ec_supported[i].curve_type == EDWARDS_CURVE) {
                    TRACE_ERROR("Edwards curves are not supported for key "
                                "type CKK_EC.\n");
            ...
            case CKK_EC_EDWARDS:
                if (der_ec_supported[i].curve_type != EDWARDS_CURVE) {
                    TRACE_ERROR("Only Edwards curves are supported for key "
                                "type CKK_EC_EDWARDS.\n");
                    return CKR_CURVE_NOT_SUPPORTED;
                }
                break;
            default:
                TRACE_ERROR("Only EC or Edwards keys are supported.\n");
                                      ...
                                      CK_BYTE *publ_tok)
{
    ...
    case CKM_ML_DSA_KEY_PAIR_GEN:
        ...
        t1_len = be16toh(*((uint16_t *)(publ_tok +
                                        CCA_QSA_EXTTOK_PUBLKEY_OFFSET +
    ...
    case CKM_ML_KEM_KEY_PAIR_GEN:
        ...
        seed_len = be16toh(*((uint16_t *)(publ_tok +
                                          CCA_QSA_EXTTOK_PUBLKEY_OFFSET +
                                          CCA_QSA_EXTTOK_SEED_OFFSET)));

        if (pk_len + seed_len != oid->len_info.ml_kem.pk_len) {
            TRACE_ERROR("CCA QSA key token has invalid key component length\n");
            return CKR_FUNCTION_FAILED;
        }
        break;
    ...
    /* Add public attributes to private and public template */
    ...
    rc = pqc_add_keyform_mode(publ_tmpl, oid, mech);
    if (rc != CKR_OK) {
        TRACE_ERROR("pqc_add_keyform_mode failed\n");
        return rc;
    }

    rc = pqc_add_keyform_mode(priv_tmpl, oid, mech);
    if (rc != CKR_OK) {
        TRACE_ERROR("pqc_add_keyform_mode failed\n");
        return rc;
    }
                                      ...
                                      CK_BYTE *priv_tok)
{
    ...
    publ_sec = &priv_tok[CCA_QSA_INTTOK_PRIVKEY_OFFSET + privsec_len];
    publsec_len = be16toh(*((uint16_t *)(publ_sec + CCA_SECTION_LEN_OFFSET)));
    if (CCA_QSA_INTTOK_PRIVKEY_OFFSET + privsec_len + publsec_len >
                                                    (int)priv_tok_len) {
        TRACE_ERROR("CCA QSA key token has invalid publ section len or "
                    "token size\n");
        return CKR_FUNCTION_FAILED;
                                        ...
                                        long *key_value_structure_length)
{
    ...
    case CKM_ML_KEM_KEY_PAIR_GEN:
        ...
        case CKP_ML_KEM_1024:
            ...
            break;
        default:
            TRACE_DEVEL("ML-KEM keyform %lu not supported by CCA\n",
                        oid->keyform);
            return CKR_KEY_SIZE_RANGE;
        }
        break;
...
static CK_RV ccatok_pqc_generate_keypair(STDLL_TokData_t *tokdata,
                                         CK_MECHANISM *mech,
                                         const struct pqc_oid *oid,
                                         TEMPLATE *publ_tmpl,
                                         TEMPLATE *priv_tmpl)
{
    ...
    if (return_code != CCA_SUCCESS) {
        TRACE_ERROR("CSNDPKB (QSA KEY TOKEN BUILD) failed. return:%ld,"
                    " reason:%ld\n", return_code, reason_code);
        if (return_code == 8 && reason_code == 998)
            return CKR_KEY_SIZE_RANGE;
    ...
    TRACE_DEVEL("Secure QSA private key token generated. size: %ld\n",
                priv_key_token_length);
...
static CK_RV cca_ml_kem_en_decapsulate_key(STDLL_TokData_t *tokdata,
                                           SESSION *sess,
                                           ...
                                           CK_OBJECT_HANDLE *phKey)
{
    ...
    CK_IBM_CCA_AES_KEY_MODE_TYPE aes_key_mode;
    unsigned char symkey_skeleton[CCA_MAX_AES_CIPHER_KEY_SIZE] = { 0 };
    long symkey_skeleton_len = sizeof(symkey_skeleton);
    long reserved_1 = 0;
    unsigned char symkey_token[CCA_MAX_AES_CIPHER_KEY_SIZE] = { 0, };
    long symkey_token_length = sizeof(symkey_token);
    unsigned int keybitsize;
    enum cca_token_type cca_keytype;
    const CK_BYTE *mkvp;
    CK_BBOOL new_mk;
    CK_RV rc = CKR_OK;
    ...
    if (!cca_pqc_strength_supported(tokdata, CKM_ML_KEM, oid->keyform)) {
        TRACE_DEVEL("PQC keyform %lu not supported by CCA\n",
                    oid->keyform);
        return CKR_KEY_SIZE_RANGE;
    }

    if (keytype != CKK_AES || keylen != AES_KEY_SIZE_256) {
        TRACE_ERROR("CCA only supports to en/decapsulate AES-256 keys\n");
        return CKR_KEY_TYPE_INCONSISTENT;
    }

    if (encapsulate && length_only) {
        *pulCiphertextLen = oid->len_info.ml_kem.ct_len;
        return CKR_OK;
    }

    if (encapsulate && *pulCiphertextLen < oid->len_info.ml_kem.ct_len) {
        *pulCiphertextLen = oid->len_info.ml_kem.ct_len;
        TRACE_ERROR("%s\n", ock_err(ERR_BUFFER_TOO_SMALL));
        return CKR_BUFFER_TOO_SMALL;
    }
    ...
    if (!encapsulate && *pulCiphertextLen != oid->len_info.ml_kem.ct_len) {
        TRACE_ERROR("%s\n", ock_err(ERR_UNWRAPPING_KEY_SIZE_RANGE));
    ...
    /* CKA_IBM_CCA_AES_KEY_MODE must be CK_IBM_CCA_AES_CIPHER_KEY */
    rc = template_attribute_get_ulong(new_key_obj->template,
                                      CKA_IBM_CCA_AES_KEY_MODE,
                                      &aes_key_mode);
    if (rc == CKR_OK && aes_key_mode != CK_IBM_CCA_AES_CIPHER_KEY) {
        TRACE_ERROR("%s\n", ock_err(ERR_ATTRIBUTE_VALUE_INVALID));
        rc = CKR_ATTRIBUTE_VALUE_INVALID;
        goto out;
    }
    aes_key_mode = CK_IBM_CCA_AES_CIPHER_KEY;
    ...
    USE_CCA_ADAPTER_START(tokdata, return_code, reason_code)
        dll_CSNBKTB2(&return_code, &reason_code, NULL, NULL,
                     ...
                     &reserved_1, NULL,
                     &reserved_1, NULL,
                     &symkey_skeleton_len, symkey_skeleton);
    USE_CCA_ADAPTER_END(tokdata, return_code, reason_code)
     ...
     *
     *  TODO: no longer needed with CCA 8.5 ?
     */
    symkey_skeleton[26] = 0x02;
    symkey_skeleton[27] = 0x02;
    ...
    if (encapsulate) {
        ...
        RETRY_NEW_MK_BLOB_START()
            dll_CSNDPKE(&return_code, &reason_code,
                        ...
                        &symkey_skeleton_len, symkey_skeleton,
                        (long *)&(opaque_attr->ulValueLen),
                        opaque_attr->pValue,
                        (long *)pulCiphertextLen, pCiphertext);
        RETRY_NEW_MK_BLOB_END(tokdata, return_code, reason_code,
    ...
    if (analyse_cca_key_token(symkey_token, symkey_token_length,
                              &cca_keytype, &keybitsize, &mkvp) == FALSE ||
        mkvp == NULL) {
    ...
    if (check_expected_mkvp(tokdata, cca_keytype, mkvp, &new_mk) != CKR_OK) {
        TRACE_ERROR("%s\n", ock_err(ERR_DEVICE_ERROR));
        rc = CKR_DEVICE_ERROR;
        goto out;
    }

    rc = cca_reencipher_created_key(tokdata, new_key_obj->template,
                                    symkey_token, symkey_token_length,
                                    new_mk, cca_keytype, FALSE);
    if (rc != CKR_OK) {
        TRACE_ERROR("cca_reencipher_created_key failed: 0x%lx\n", rc);
        goto out;
    }
    ...
    /* Add CKA_IBM_OPAQUE */
    rc = build_update_attribute(new_key_obj->template, CKA_IBM_OPAQUE,
                                symkey_token, symkey_token_length);
    if (rc != CKR_OK) {
    ...
    rc = build_update_attribute(new_key_obj->template, CKA_VALUE,
                                dummy, keylen);
    if (rc != CKR_OK) {
        TRACE_ERROR("%s build_update_attribute failed with rc=0x%lx\n",
                    __func__, rc);
    ...
    /* Add CKA_VALUE_LEN */
    rc = build_update_attribute(new_key_obj->template, CKA_VALUE_LEN,
                                (CK_BYTE *)&keylen, sizeof(keylen));
    if (rc != CKR_OK) {
        TRACE_ERROR("%s build_update_attribute failed with rc=0x%lx\n",
                    __func__, rc);
        goto out;
    ...
    rc = build_update_attribute(new_key_obj->template,
                                CKA_IBM_CCA_AES_KEY_MODE,
                                (CK_BYTE *)&aes_key_mode,
                                sizeof(aes_key_mode));
    if (rc != CKR_OK) {
        TRACE_ERROR("%s build_update_attribute failed with rc=0x%lx\n",
                    __func__, rc);
                                            ...
                                            CK_OBJECT_HANDLE *phKey)
{
    ...
    return cca_ml_kem_en_decapsulate_key(tokdata, sess, TRUE, length_only,
                                         oid, key_obj,
                                            ...
                                            CK_OBJECT_HANDLE *phKey)
{
    ...
    return cca_ml_kem_en_decapsulate_key(tokdata, sess, FALSE, FALSE,
                                         oid, key_obj,
                                         pTemplate, ulAttributeCount,
                                        ...
                                        long *key_value_structure_length)
{
    ...
    if (private_key) {
        ...
        rc = pqc_pack_priv_key(templ, oid, mech,
                                ...
                                CK_KEY_TYPE keytype)
{
    ...
    CK_BBOOL new_mk;
    CK_MECHANISM_TYPE mech;
    CK_ATTRIBUTE_TYPE priv_seed_attr = (CK_ATTRIBUTE_TYPE)-1;
    ...
    if (rc == TRUE) {
        ...
        /* check PQC variant and add keyform and mode attributes */
        t = opaque_attr->pValue;
        privsec_len = be16toh(*((uint16_t *)
                                    (t + CCA_QSA_INTTOK_PRIVKEY_OFFSET + 2)));
        ...
        if (!cca_pqc_strength_supported(tokdata, mech, oid->keyform)) {
            TRACE_DEVEL("PQC keyform %lu not supported by CCA\n",
                        oid->keyform);
            return CKR_KEY_SIZE_RANGE;
        }

        rc = pqc_add_keyform_mode(priv_templ, oid, mech);
        if (rc != CKR_OK) {
            TRACE_ERROR("pqc_add_keyform_mode failed\n");
            return rc;
        }
        ...
        long private_key_name_length, key_token_length, target_key_token_length;
        long return_code, reason_code, rule_array_count, exit_data_len = 0;
        long key_value_structure_length, param1=0, regeneration_data_len;
        unsigned char rule_array[CCA_RULE_ARRAY_SIZE] = { 0, };
        unsigned char key_value_structure[CCA_KEY_VALUE_STRUCT_SIZE] = { 0, };
        unsigned char private_key_name[CCA_PRIVATE_KEY_NAME_SIZE] = { 0, };
        unsigned char key_token[CCA_KEY_TOKEN_SIZE] = { 0, };
        ...
        if (priv_seed_attr != (CK_ATTRIBUTE_TYPE)-1 &&
            template_attribute_find(priv_templ, priv_seed_attr,
                                    &priv_seed) == TRUE &&
            priv_seed->ulValueLen > 0 && priv_seed->pValue != NULL) {
            ...
            if (priv_seed->ulValueLen != priv_seed_len) {
                TRACE_ERROR("Private seed length is invalid\n");
                return CKR_ATTRIBUTE_VALUE_INVALID;
            }
        ...
        if (priv_seed != NULL) {
            /* Generate a PKA key using the seed as regeneration data */
            rule_array_count = 2;
            memcpy(rule_array, "MASTER  RAWSEED ", CCA_KEYWORD_SIZE * 2);
            regeneration_data_len = priv_seed->ulValueLen;
            target_key_token_length = CCA_KEY_TOKEN_SIZE;
            ...
            USE_CCA_ADAPTER_START(tokdata, return_code, reason_code)
                dll_CSNDPKG(&return_code, &reason_code,
                            &exit_data_len, exit_data,
                            &rule_array_count, rule_array,
                            &regeneration_data_len, priv_seed->pValue,
                            &key_token_length, key_token,
                            transport_key_identifier,
                            &target_key_token_length, target_key_token);
            USE_CCA_ADAPTER_END(tokdata, return_code, reason_code)
            ...
            if (return_code != CCA_SUCCESS) {
                TRACE_ERROR("CSNDPKG (QSA KEY RE-GENERATE) failed."
                            " return:%ld, reason:%ld\n",
                            return_code, reason_code);
                if (return_code == 8 && reason_code == 998)
                    return CKR_KEY_SIZE_RANGE;
        ...
        case CKK_IBM_PQC_DILITHIUM:
        case CKK_IBM_ML_DSA:
            cleanse_attribute(priv_templ, CKA_IBM_ML_DSA_SEED);
            cleanse_attribute(priv_templ, CKA_IBM_ML_DSA_TR);
            cleanse_attribute(priv_templ, CKA_IBM_ML_DSA_S1);
            cleanse_attribute(priv_templ, CKA_IBM_ML_DSA_S2);
            cleanse_attribute(priv_templ, CKA_IBM_ML_DSA_T0);
            cleanse_attribute(priv_templ, CKA_IBM_ML_DSA_PRIVATE_SEED);
            break;
        case CKK_IBM_ML_KEM:
                               ...
                               CK_KEY_TYPE keytype)
{
    ...
    if (rc == TRUE) {
        ...
        /* check PQC variant and add keyform and mode attributes */
        t = opaque_attr->pValue;
        pubsec_len = be16toh(*((uint16_t *)(t +
                             CCA_QSA_EXTTOK_PUBLKEY_OFFSET + 2)));
        if (pubsec_len + CCA_QSA_EXTTOK_PUBLKEY_OFFSET >
                                          (int)opaque_attr->ulValueLen) {
            TRACE_DEVEL("CCA QSA key token has invalid pub section len or "
                        "token size\n");
            return CKR_FUNCTION_FAILED;
        }
        ...
        rc = pqc_add_keyform_mode(pub_templ, oid, mech);
        if (rc != CKR_OK) {
            TRACE_ERROR("pqc_add_keyform_mode failed\n");
            return rc;
        }
...
CK_RV token_specific_object_add(STDLL_TokData_t *tokdata, SESSION *sess, OBJECT *object)
{
    ...
    case CKK_ML_KEM:
        ...
        case CKO_PUBLIC_KEY:
            ...
            TRACE_INFO("PQC public key imported\n");
            break;
        case CKO_PRIVATE_KEY:
            // do import keypair and create opaque object
            ...
            TRACE_INFO("PQC private key imported\n");
            break;
        default:
            TRACE_ERROR("%s\n", ock_err(ERR_KEY_TYPE_INCONSISTENT));
            return CKR_KEY_TYPE_INCONSISTENT;
