                           ...
                           CK_OBJECT_HANDLE *phKey)
{
    ...
    CK_ULONG derive_tmpl_num = 0, derive_key_len;
    CK_IBM_CCA_AES_KEY_MODE_TYPE derive_cca_key_mode;
    CK_RV rc, rc2;

    CK_ATTRIBUTE derive_tmpl[3] = { 0 };
    CK_ATTRIBUTE ec_publ_key_tmpl[] = {
    ...
    CK_ATTRIBUTE ec_priv_key_tmpl[] = {
        { CKA_DERIVE_TEMPLATE, derive_tmpl, 0 }, /* must be first */
        { CKA_HIDDEN, &ck_true, sizeof(ck_true) },
        { CKA_SENSITIVE, &ck_true, sizeof(ck_true) },
        { CKA_TOKEN, &ck_false, sizeof(ck_false) },
        { CKA_PRIVATE, &ck_true, sizeof(ck_true) },
    ...
    /* Supply CKA_DERIVE_TEMPLATE contents (required by CCA token) */
    rc = get_ulong_attribute_by_type(pTemplate, ulAttributeCount,
                                     CKA_KEY_TYPE, &derive_key_type);
    if (rc == CKR_OK) {
        derive_tmpl[derive_tmpl_num].type = CKA_KEY_TYPE;
        derive_tmpl[derive_tmpl_num].pValue = &derive_key_type;
        derive_tmpl[derive_tmpl_num].ulValueLen = sizeof(derive_key_type);
        derive_tmpl_num++;
    }

    rc = get_ulong_attribute_by_type(pTemplate, ulAttributeCount,
                                     CKA_VALUE_LEN, &derive_key_len);
    if (rc == CKR_OK) {
        derive_tmpl[derive_tmpl_num].type = CKA_VALUE_LEN;
        derive_tmpl[derive_tmpl_num].pValue = &derive_key_len;
        derive_tmpl[derive_tmpl_num].ulValueLen = sizeof(derive_key_len);
        derive_tmpl_num++;
    }
    ...
    rc = get_ulong_attribute_by_type(pTemplate, ulAttributeCount,
                                     ...
                                     &derive_cca_key_mode);
    if (rc == CKR_OK) {
        derive_tmpl[derive_tmpl_num].type = CKA_IBM_CCA_AES_KEY_MODE;
        derive_tmpl[derive_tmpl_num].pValue = &derive_cca_key_mode;
        derive_tmpl[derive_tmpl_num].ulValueLen = sizeof(derive_cca_key_mode);
        derive_tmpl_num++;
    }

    ec_priv_key_tmpl[0].ulValueLen = derive_tmpl_num * sizeof(CK_ATTRIBUTE);
