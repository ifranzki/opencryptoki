                              ...
                              CK_BBOOL count_statistics)
{
    ...
    if (token_specific.t_encapsulate_key != NULL) {
        rc = token_specific.t_encapsulate_key(tokdata, sess, length_only,
                                              mech, public_key_obj,
                                              new_attrs, new_attr_count,
                                              pCiphertext, pulCiphertextLen,
                                              phKey);
        if (rc != CKR_OK) {
            TRACE_ERROR("token specific encapsulate key function failed\n");
        }
                              ...
                              CK_BBOOL count_statistics)
{
    ...
    if (token_specific.t_decapsulate_key != NULL) {
        rc = token_specific.t_decapsulate_key(tokdata, sess, mech,
                                              private_key_obj,
                                              new_attrs, new_attr_count,
                                              pCiphertext, ulCiphertextLen,
                                              phKey);
        if (rc != CKR_OK) {
            TRACE_ERROR("token specific decapsulate key function failed\n");
        }
