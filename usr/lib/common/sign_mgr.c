                    ...
                    CK_BYTE *out_data, CK_ULONG *out_data_len)
{
    ...
    case CKM_IBM_ML_DSA:
        return ibm_ml_dsa_sign(tokdata, sess, length_only, ctx,
                               in_data, in_data_len, out_data, out_data_len);
    case CKM_ML_DSA:
        return ml_dsa_sign(tokdata, sess, length_only, ctx,
                           in_data, in_data_len, out_data, out_data_len);
    case CKM_HASH_ML_DSA:
    case CKM_HASH_ML_DSA_SHA224:
    case CKM_HASH_ML_DSA_SHA256:
    ...
    case CKM_HASH_ML_DSA_SHAKE128:
    case CKM_HASH_ML_DSA_SHAKE256:
        return ml_dsa_hash_sign(tokdata, sess, length_only, ctx,
                                in_data, in_data_len, out_data, out_data_len);
    default:
        TRACE_ERROR("%s\n", ock_err(ERR_MECHANISM_INVALID));
                          ...
                          CK_BYTE *signature, CK_ULONG *sig_len)
{
    ...
    case CKM_HASH_ML_DSA_SHAKE128:
    case CKM_HASH_ML_DSA_SHAKE256:
        return ml_dsa_hash_sign_final(tokdata, sess, length_only,  ctx,
                                      signature, sig_len);
    default:
        TRACE_ERROR("%s\n", ock_err(ERR_MECHANISM_INVALID));
        return CKR_MECHANISM_INVALID;
