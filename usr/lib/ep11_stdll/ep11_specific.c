                                     ...
                                     CK_SESSION_HANDLE h)
{
    ...
    /* Decode SPKI */
    rc = ber_decode_SPKI(spki, spki_len, &oid, &oid_len, &parm, &parm_len, &key,
            &bit_str_len);
    if (rc != CKR_OK) {
        TRACE_ERROR("%s read key from SPKI failed with rc=0x%lx\n", __func__,
                rc);
    ...
    rc = ber_decode_SEQUENCE(key, bit_str_len, &data, &data_len, &field_len);
    if (rc != CKR_OK) {
        TRACE_ERROR("%s read sequence failed with rc=0x%lx\n", __func__, rc);
        goto error;
    }
    ...
    rho_len = bit_str_len - field_len + data_len;
    rc = ber_decode_BIT_STRING(rho, rho_len, &data, &data_len, &field_len);
    if (rc != CKR_OK) {
    ...
    /* Decode t1 */
    t1 = rho + field_len;
    t1_len = rho_len - field_len;
    rc = ber_decode_BIT_STRING(t1, t1_len, &data, &data_len, &field_len);
    if (rc != CKR_OK) {
