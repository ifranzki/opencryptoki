                                                      ...
                                                      TEMPLATE *priv_tmpl)
{
    ...
    if (rc != 0) {
        switch (rc) {
        case EPERM:
            TRACE_ERROR("ica_ed_x_key_gen failed with rc=EPERM, "
                        "probably curve not supported by openssl.\n");
            ret = CKR_CURVE_NOT_SUPPORTED;
            break;
        case ENODEV:
            TRACE_ERROR("%s\n", ock_err(ERR_FUNCTION_NOT_SUPPORTED));
    ...
    /* Return public key (D) via CKA_EC_POINT */
                                                 ...
                                                 TEMPLATE *priv_tmpl)
{
    ...
    if (ica_data->ica_ec_edwards_keygen_available) {
        rc = ica_specific_ec_edwards_montgomery_generate_keypair(tokdata,
                                                                 publ_tmpl,
                                                                 priv_tmpl);
        if (rc == CKR_FUNCTION_NOT_SUPPORTED)
                                                      ...
                                                      int *nid)
{
    ...
    if (rc != 0) {
        ...
        case ENODEV:
            ...
            break;
        case EPERM:
            TRACE_ERROR("ica_ed_x_key_set() failed with rc=EPERM, probably "
                        "curve not supported by openssl.\n");
            ret = CKR_CURVE_NOT_SUPPORTED;
            break;
        default:
                                                     ...
                                                     int *nid)
{
    ...
    /* Get publuc key from template via CKA_EC_POINT */
    ret = template_attribute_get_non_empty(key_obj->template, CKA_EC_POINT,
                                           &attr);
    if (ret != CKR_OK) {
        TRACE_ERROR("%s\n", ock_err(ERR_TEMPLATE_INCOMPLETE));
    ...
    /* Initialize Ed/X with public key (D) */
                                            ...
                                            CK_MECHANISM *mech)
{
    ...
    if (ex_data->ed_x_ctx == NULL) {
        ...
        ret = ica_build_ec_edwards_montgomery_pub_ctx(key_obj,
                                                      ...
                                                      &ex_data->ed_x_nid);
        if (ret != CKR_OK) {
            TRACE_ERROR("ica_build_ec_edwards_pub_ctx() failed with "
                        "rc = 0x%lx. \n", ret);
            goto end;
        }
    }
