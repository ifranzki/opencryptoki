...
#define IMPORT_KEYTYPES                                                        \
    ...
    { .value = "ec", .args = p11sak_import_asym_args,                          \
      .private = { .ptr = &p11sak_ec_keytype }, },                             \
    { .value = "ec-edwards", .args = p11sak_import_asym_args,                  \
      .private = { .ptr = &p11sak_ec_edwards_keytype }, },                     \
    { .value = "ec-montgomery", .args = p11sak_import_asym_args,               \
      .private = { .ptr = &p11sak_ec_montgomery_keytype }, },                  \
                                   ...
                                   CK_ATTRIBUTE **attrs, CK_ULONG *num_attrs)
{
    ...
    case EVP_PKEY_EC:
        if (keytype->type != CKK_EC) {
            warnx("PEM file '%s' contain an EC %s key, but a %s %s key is "
    ...
    case EVP_PKEY_ED448:
        if (keytype->type != CKK_EC && keytype->type != CKK_EC_EDWARDS) {
            warnx("PEM file '%s' contain an EC-Edwards %s key, but a %s %s "
    ...
    case EVP_PKEY_X448:
        if (keytype->type != CKK_EC && keytype->type != CKK_EC_MONTGOMERY) {
            warnx("PEM file '%s' contain an EC-Montgomery %s key, but a %s %s "
                  "key is expected.", opt_file, private ? "private" : "public",
                  keytype->name, private ? "private" : "public");
            return CKR_ARGUMENTS_BAD;
        }
