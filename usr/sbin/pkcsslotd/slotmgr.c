...
int check_token_group(struct group *tok_grp)
{
    ...
    if (getgrnam_r(PKCS_GROUP, &grp_buf, buff, buf_size, &pkcs11_grp) != 0) {
        ...
        fprintf(stderr, "'%s' group does not exist [errno=%s].\n", PKCS_GROUP,
                strerror(err));
       rc = err;
       goto done;
    }
    ...
    for (i = 0; tok_grp->gr_mem[i] != NULL; i++) {
        ...
        if (!found) {
            fprintf(stderr, "User '%s' is member of the token group '%s', but "
...
int chk_create_tokdir(Slot_Info_t_64 *psinfo)
{
     ...
     * specified, the token directory name is not know, thus we can not check
     * or create it.
     */
    ...
    if (rc != 0 && err == ENOENT) {
        if (strcmp(tokgroup, PKCS_GROUP) != 0) {
            fprintf(stderr,
                    ...
                    "manually with the proper owners.\n",
                    tokendir, PKCS_GROUP);
            umask(proc_umask);
            return EACCES;
        }
