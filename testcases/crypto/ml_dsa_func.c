...
CK_HASH_SIGN_ADDITIONAL_CONTEXT hash_no_ctx_preferred_sha256 =
                                { CKH_HEDGE_PREFERRED, NULL, 0, CKM_SHA256 };
CK_HASH_SIGN_ADDITIONAL_CONTEXT hash_ctx_preferred_sha3_512 =
...
const _signVerifyParam signVerifyInput[] = {
    ...
    {"CKM_ML_DSA 3000 bytes message",
            {CKM_ML_DSA, NULL, 0}, 3000, 0, { 0, }},
    ...
    {"CKM_ML_DSA with context, hedge=preferred",
            {CKM_ML_DSA, &with_ctx_preferred, sizeof(with_ctx_preferred)},
             32, 0, { 0, }},
    {"CKM_HASH_ML_DSA 32 bytes prehashed message (SHA256)",
            {CKM_HASH_ML_DSA, &hash_no_ctx_preferred_sha256,
             sizeof(hash_no_ctx_preferred_sha256)}, 32, 0, { 0, }},
    {"CKM_HASH_ML_DSA 64 bytes prehashed message (SHA3-512) with context",
            {CKM_HASH_ML_DSA, &hash_ctx_preferred_sha3_512,
             sizeof(hash_ctx_preferred_sha3_512)}, 64, 0, { 0, }},
                          ...
                          CK_OBJECT_HANDLE publ_key)
{
    ...
    if (rc != CKR_OK) {
        if (rc == CKR_MECHANISM_PARAM_INVALID &&
            ...
            ((CK_SIGN_ADDITIONAL_CONTEXT *)mech->pParameter)->hedgeVariant
                                    == CKH_DETERMINISTIC_REQUIRED) {
            testcase_skip("Sign with CKH_DETERMINISTIC_REQUIRED not supported");
            rc = CKR_OK;
            goto testcase_cleanup;
        }
        if (rc == CKR_MECHANISM_PARAM_INVALID &&
            mech->pParameter != NULL &&
            ((CK_SIGN_ADDITIONAL_CONTEXT *)mech->pParameter)->ulContextLen
                                                                    > 0) {
    ...
    if (chunks == 0) {
        ...
        if (rc != CKR_OK) {
            if (rc == CKR_MECHANISM_PARAM_INVALID &&
                ...
                ((CK_SIGN_ADDITIONAL_CONTEXT *)mech->pParameter)->hedgeVariant
                                        == CKH_DETERMINISTIC_REQUIRED) {
                testcase_skip("Sign with hedge type CKH_DETERMINISTIC_REQUIRED "
                              "is not supported");
                rc = CKR_OK;
                goto testcase_cleanup;
            }
        ...
        if (rc != CKR_OK) {
            ...
            if (rc == CKR_MECHANISM_PARAM_INVALID &&
                ...
                ((CK_SIGN_ADDITIONAL_CONTEXT *)mech->pParameter)->ulContextLen
                                                                        > 0) {
                testcase_skip("C_SignFinal with non-empty context not "
                              "supported");
                rc = CKR_OK;
                goto testcase_cleanup;
            }
