...
/**
 * Support for ML-KEM keys and KEM
 * with oid = 2.16.840.1.101.3.4.4.xxx
 */

typedef struct variant_info {
    const char *name;
    CK_ML_KEM_PARAMETER_SET_TYPE parameter_set;
} _variant_info;
...
const _kemParam kemInput[] = {
    ...
    { CKK_GENERIC_SECRET, 32 },
};

const CK_ULONG num_kem_inputs = sizeof(kemInput) / sizeof(_kemParam);
                             ...
                             CK_ULONG secret_key_len)
{
    ...
    if (rc != CKR_OK) {
        if (rc == CKR_KEY_TYPE_INCONSISTENT) {
            testcase_notice("Slot %u does not support en-/decapsulating "
                            "keys of type 0x%lx", (unsigned int)SLOT_ID,
                            key_type);
            goto testcase_cleanup;
        }
        testcase_error("C_EncapsulateKey (size query) rc=%s", p11_get_ckr(rc));
        goto testcase_cleanup;
    }
    ...
    if (secret_key1 != CK_INVALID_HANDLE) {
        ...
        case CKK_GENERIC_SECRET:
            rc = run_HMACSign(session, secret_key1, secret_key2,
                              secret_key_len);
            if (rc != CKR_OK) {
                testcase_fail("Derived keys are not usable or not the same: %s",
                              p11_get_ckr(rc));
...
CK_RV run_GenerateMLKEMKeyPairKEM(void)
{
    ...
    for (i = 0; i < num_variants; i++) {
        ...
        testcase_begin("Starting ML-KEM generate key pair with %s.",
                       variants[i].name);
        ...
        for (j = 0; j < num_kem_inputs; j++) {
            ...
            if (rc == CKR_MECHANISM_INVALID ||
                ...
                testcase_skip("run_EnDecapsulateMLKEM with %s (index %lu).",
                               variants[i].name, j);
            } else if (rc != 0) {
                testcase_new_assertion();
                testcase_fail("run_EnDecapsulateMLKEM with %s (index %lu) "
                              "failed.", variants[i].name, j);
                goto next;
            } else {
                testcase_new_assertion();
                testcase_pass("Encapsulate & Decapsulate (KEM) with %s "
                              "(index %lu) passed.", variants[i].name, j);
            }
        }
...
CK_RV run_ImportMLKEMKeyPairSignVerify(void)
{
    ...
    if (!mech_supported(SLOT_ID, CKM_ML_KEM)) {
        testcase_skip("Slot %u doesn't support CKM_ML_KEM",
                      (unsigned int) SLOT_ID);
        rc = CKR_OK;
        goto testcase_cleanup;
    }
    ...
    for (i = 0; i < ML_KEM_TV_NUM; i++) {
        ...
        testcase_begin("Starting ML-KEM import key pair, %s index=%lu",
                       ml_kem_tv[i].name, i);

        /* Create ML-KEM private key */
        rc = create_ML_KEM_PrivateKey(session,
                                      ml_kem_tv[i].parameter_set,
                                      ml_kem_tv[i].priv,
                                      ml_kem_tv[i].priv_len,
        ...
        if (rc != CKR_OK) {
            if (rc == CKR_KEY_SIZE_RANGE) {
                ...
                testcase_skip("ML-KEM key import is not allowed by policy");
                continue;
            }
            testcase_new_assertion();
            testcase_fail("C_CreateObject (ML-KEM Private Key) failed at "
                          "i=%lu, rc=%s", i, p11_get_ckr(rc));
            goto testcase_cleanup;
        }
        testcase_new_assertion();
        testcase_pass("*Import ML-KEM private key (%s) index=%lu passed.",
                      ml_kem_tv[i].name, i);

        /* Create ML-KEM public key */
        rc = create_ML_KEM_PublicKey(session,
                                     ml_kem_tv[i].parameter_set,
                                     ml_kem_tv[i].pub,
                                     ml_kem_tv[i].pub_len,
                                     &publ_key);
        if (rc != CKR_OK) {
            ...
            testcase_new_assertion();
            testcase_fail("C_CreateObject (ML-KEM Public Key) failed at "
                          "i=%lu, rc=%s", i, p11_get_ckr(rc));
            goto testcase_cleanup;
        }
        testcase_new_assertion();
        testcase_pass("*Import ML-KEM public key (%s) index=%lu passed.",
                      ml_kem_tv[i].name, i);

        /* Test Encapsulate/decapsulate (KEM) */
        rc = run_EnDecapsulateMLKEM(session, priv_key, publ_key, CKK_AES, 32);
        if (rc == CKR_MECHANISM_INVALID || rc == CKR_KEY_SIZE_RANGE) {
                ...
                CK_OBJECT_HANDLE secret_key, CK_OBJECT_HANDLE *unwrapped_key)
{
    ...
    CK_BYTE unwrap_label[] = "unwrapped_private_ML_KEM_Key";
    CK_BYTE subject[] = {0};
    CK_BYTE id[] = { 123 };
    CK_BBOOL true = TRUE;
    CK_RV rc;
...
CK_RV run_TransferMLKEMKeyPairSignVerify(void)
{
    ...
    for (i = 0; i < ML_KEM_TV_NUM; i++) {
        ...
        testcase_begin("Starting ML-KEM transfer key pair, %s index=%lu.",
                       ml_kem_tv[i].name, i);
        ...
        testcase_new_assertion();
        testcase_pass("*Import I ML-KEM private key (%s) index=%lu passed.",
                      ml_kem_tv[i].name, i);
        ...
        /* Wrap ML-KEM private key with secret key */
        ...
        testcase_pass("*Wrap ML-KEM private key (%s) index=%lu passed.",
                      ml_kem_tv[i].name, i);
        ...
        testcase_pass("*Unwrap ML-KEM private key (%s) index=%lu passed.",
                      ml_kem_tv[i].name, i);
        ...
        /* Test Encapsulate/decapsulate (KEM) */
        rc = run_EnDecapsulateMLKEM(session, unwrapped_key, publ_key,
                                    CKK_AES, 32);
        if (rc == CKR_MECHANISM_INVALID) {
            testcase_skip("run_EnDecapsulateMLKEM index=%lu.", i);
        } else if (rc != 0) {
