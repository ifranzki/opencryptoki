    ...
    for (i = 0; i < NUMED; i++) {
        ...
        for (j = 0;
            ...
            rc = run_GenerateSignVerifyECC(session,
                                           ...
                                           der_ec_edwards_supported[i].size);
            if (rc != 0) {
                testcase_fail("run_GenerateSignVerifyECC failed index=%lu.",
                              j);
                goto testcase_cleanup;
            }
...
CK_RV run_ImportEdwardsKeyPairSignVerifyKAT(void)
{
    ...
    CK_SESSION_HANDLE session;
    CK_BYTE user_pin[PKCS11_MAX_PIN_LEN];
    CK_ULONG user_pin_len, i, signaturelen;
    CK_FLAGS flags;
    CK_MECHANISM_INFO mech_info;
    ...
    for (i = 0; i < NUMED; i++) {
        ...
        if (mech.mechanism == CKM_EC_EDWARDS_KEY_PAIR_GEN) {
            ...
            rc = run_GenerateSignVerifyECC(session,
                                           ...
                                           der_ec_edwards_supported[i].size);
             if (rc != 0) {
                 testcase_fail("run_GenerateSignVerifyECC failed index=0.");
                 goto testcase_cleanup;
             }
