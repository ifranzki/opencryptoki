...
Implements the EC-DH (CKM_ECDH1_DERIVE) variant.  PKCS-DH and SSL/TLS
derive variants are not implemented.

Request wire format (from icsf_derive_key() in icsf.c):

    DVKInput ::= SEQUENCE {
        attrList        Attributes,         -- written by icsf_ber_put_attribute_list
        parmsListChoice [2] SEQUENCE {      -- EC-DH choice (context-constructed tag 2)
    ...
    Rule array: "EC-DH   " (8-byte padded keyword)
    Handle:     44-byte handle of the base (private) EC key
...
from pkcs11_const import (
    CKA_KEY_TYPE, CKA_VALUE_LEN, CKA_TOKEN, CKA_EC_PARAMS,
    CKK_AES, CKK_DES, CKK_DES2, CKK_DES3,
...
def handle_dvk(store, request):
    ...
    # Only EC-DH is implemented
    rules = [r.upper() for r in request.rule_array]
    if 'EC-DH' not in rules:
        logger.warning('DVK: unsupported rule array %r (only EC-DH implemented)',
                       request.rule_array)
        return encode_response(
            request.handle, RC_ERROR, RSN_MECH_INVALID, ICSF_TAG_CSFPDVK, b'')

    # Locate the base (EC private) key object
    ...
    # -----------------------------------------------------------------------
    # Parse DVKInput from service_data:
    #   1. attribute list SEQUENCE
    #   2. context-constructed [2] SEQUENCE { kdfCode, sharedData, publicValue }
    # -----------------------------------------------------------------------
    derived_attrs = []
    kdf_code     = CKD_NULL
    shared_data  = b''
    ...
    # Decode the peer public value with full curve-aware logic: DER-wrapped or
    # raw, uncompressed/compressed/hybrid/bare X||Y (with or without format byte).
    # -----------------------------------------------------------------------
    ...
    # -----------------------------------------------------------------------
    # Apply KDF to produce symmetric key material
    # -----------------------------------------------------------------------
    ...
    # Determine token vs session object
    ...
    # Strip CKA_VALUE_LEN from caller attrs (complete_key_attrs re-derives it)
    ...
    # DVKOutput for EC-DH: service data is empty ([2] NULL in ICSF spec)
