...
checks that icsftok_set_attribute_value performs in icsf_specific.c):
  ...
  1. If CKA_MODIFIABLE is FALSE on the object: rc=8, reason=3020
     (mapped to CKR_ACTION_PROHIBITED in icsftok_set_attribute_value).
     The mock uses a dedicated reason outside the icsf_to_ock_err table so
     that enforcement in icsf_specific.c catches it before reaching here, but
     the server enforces it as a safety net as well.

  2. CKA_MODIFIABLE is read-only after creation: rc=8, reason=3034
     (mapped to CKR_ATTRIBUTE_READ_ONLY).
...
RSN_ACTION_PROHIB   = 3020   # CKA_MODIFIABLE=FALSE (→ CKR_ACTION_PROHIBITED)
RSN_ATTR_READ_ONLY  = 3034   # attribute is read-only (→ CKR_ATTRIBUTE_READ_ONLY)
RSN_ATTR_READ_ONLY2 = 3035   # same mapping, alternate code used for copyable
...
def handle_sav(store, request):
    ...
    if modifiable is False:
        ...
        return encode_response(
            request.handle, RC_ERROR, RSN_ACTION_PROHIB, ICSF_TAG_CSFPSAV, b'')
