...
def handle_skd(store, request):
    ...
    try:
        pos = 0
        tag, init_vec, pos    = _decode_tlv(request.service_data, pos)  # [0]
        tag, chain_in, pos    = _decode_tlv(request.service_data, pos)  # [2]
        tag, cipher_text, pos = _decode_tlv(request.service_data, pos)  # [3]
        tag, req_len_val, pos = _decode_tlv(request.service_data, pos)  # [4]
