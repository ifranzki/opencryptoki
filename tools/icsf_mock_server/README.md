...
| `CSFPPKS` |  9 | Private key sign / RSA decrypt | Real RSA/ECDSA via libcrypto |
| `CSFPPKV` | 10 | Public key verify / RSA encrypt | Real RSA/ECDSA verification via libcrypto |
| `CSFPHMG` |  6 | HMAC generate | Real `hmac.new()` using stdlib |
| `CSFPHMV` |  7 | HMAC verify | Real compare_digest; fails on mismatch |
| `CSFPOWH` |  8 | One-way hash (sign/verify) | Real hash via libcrypto EVP_DigestSign/Verify |
...
SKE/SKD use real AES/3DES (ECB, CBC, CBC-PAD) via OpenSSL `libcrypto` through
`ctypes` — no third-party Python packages required.  Known-answer tests (KATs)
with published NIST vectors pass.  WPK/UWK use the same AES-CBC-PAD path for
key wrap/unwrap.  DVK uses `EVP_PKEY_derive` for real ECDH.
