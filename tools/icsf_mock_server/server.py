...
def _handle_bind(msg_id, op_val):
    ...
    if tag == CTX_PRIM_0:
        ...
        password = auth_val.decode('utf-8', errors='replace')
...
def _parse_search_attrs(op_val):
    ...
    # We need to skip: baseObject, scope, derefAliases, sizeLimit, timeLimit,
    # typesOnly, filter — all before the AttributeSelection at the end.
    # For simplicity just return [] (= return all attributes).
    return []
...
class ICSFMockServer:
    ...
    def __init__(self, host='127.0.0.1', port=1389,
        ...
        if tls_cert and tls_key:
            ...
            # Accept client certs (SASL EXTERNAL) but don't require them
    ...
    def _seed_object(self, spec):
        ...
        spec keys:
          ...
          class   — one of: aes, des3, rsa-pub, rsa-priv, ec-pub, ec-priv, cert
                    (default: aes)
        ...
        if cls == 'aes':
            ...
            if cls == 'data':
                logger.error('Pre-seeding class %r is not supported: only key '
                             ...
                             'Use aes, des3, rsa-pub, rsa-priv, ec-pub, ec-priv, '
                             'or cert.',
                             cls)
            else:
...
def main():
    ...
    ap.add_argument('--cert',    default=None,
                    help='PEM certificate file (enables TLS / SASL EXTERNAL)')
    ap.add_argument('--key',     default=None,
                    help='PEM private key file (requires --cert)')
    ...
    ap.add_argument('--object',  action='append', default=[],
                    ...
                    help='Pre-seed an object (repeatable). '
                         ...
                         'CLASS is one of: aes, des3, rsa-pub, rsa-priv, '
